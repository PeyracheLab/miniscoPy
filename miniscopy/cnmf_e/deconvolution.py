        ...
        if method == 'cvx':
            ...
            if p == 1:
                if bl is None:
                    ...
                    c, sp, bl, g, lam = constrained_oasisAR1(fluor.astype(np.float32), g[0], sn, optimize_b=True, b_nonneg=bas_nonneg,optimize_g=optimize_g, penalty=penalty)
                else:
                    c, sp, _, g, lam = constrained_oasisAR1((fluor - bl).astype(np.float32), g[0], sn, optimize_b=False, penalty=penalty)
...
def estimate_parameters(
    fluor, p=2, sn=None, g=None, range_ff=[0.25, 0.5],
...
def deconvolve_ca(y=[], options=None, **args):
    ...
    if options['approach'].lower() == 'constrained foopsi':
        # constrained foopsi        
        c, baseline, c1, g, sn, spike, lam_ = constrained_foopsi(y, options['bl'], options['c1'],
